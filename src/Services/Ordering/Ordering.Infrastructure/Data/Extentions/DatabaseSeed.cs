namespace Ordering.Infrastructure.Data.Extensions
{
    public class DatabaseSeed
    {
        ...
        public static async Task GenerateCustomers(ApplicationDbContext context, int count)
        {
            if (!context.Customers.Any())
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    var email = $"{firstName.ToLower()}.{lastName.ToLower()}{random.Next(100, 999)}@{GetRandomEmailDomain(random)}";
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (categoryIndex != 2) // If not already ordering accessories
                    {
                        var accessories = products
                            .Where(p => p.Name.Contains("Keyboard") || p.Name.Contains("Mouse") || p.Name.Contains("Headset"))
                            .OrderBy(_ => random.Next())
        ...
        private static Address GenerateRandomAddress(Random random)
        {
            ...
            var street = $"{streetNumber} {LastNames[random.Next(LastNames.Length)]} {streetTypes[random.Next(streetTypes.Length)]}";
        ...
        private static Payment GenerateRandomPayment(Random random)
        {
            ...
            var cardNumber = cardTypes[random.Next(cardTypes.Length)] + new string(Enumerable.Range(0, 15).Select(_ => random.Next(10).ToString()[0]).ToArray());
